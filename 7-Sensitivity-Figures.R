...
mdat
write_csv(mdat, "~/Projects/abm_IUU_simulation/data/sens/v0.51/missing_agents_sep.csv")
...
# files <- list.files("~/Projects/abm_IUU_simulation/data/sens/v0.51/", full.names = TRUE, pattern = "csv")
# 
# bdat <- rbindlist(lapply(files, read_csv))
# bdat <- as.data.frame(bdat)
# 
# 
# mdat <- data.frame(nagents = rep(seq(2, 99, 1), each = 50),
#                    sep_ie = rep(seq(0.01, .50, .01)))
# bdat <- select(bdat, nagents, sep_ie, ks_mean, ks_kurt)
# mdat <- left_join(mdat, bdat, by = c("nagents", "sep_ie"))
# mdat <- mdat[which(is.na(mdat$mean_ks) | is.na(mdat$kurt_ks)), ]
# write_csv(mdat, "~/Projects/abm_IUU_simulation/data/sens/v0.51/missing_agents_sep.csv")
# 
